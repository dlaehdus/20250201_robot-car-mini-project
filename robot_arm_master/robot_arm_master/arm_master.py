...
CURRENT_LIMITS = [[-300, 300], [-800, 800], [-800, 800], [-300, 300], [-300, 300], [-100, 100]]  # 전류 제한 값 (예시: -800 ~ 800mA, 모터별 조정 가능)
NORMAL_CURRENTS = [0, 0, 0, 0, 0, 0]  # 각 모터별 정상 동작시 전류
STIFF_CURRENT = 500  # 위치 제한 초과 시 또는 slave 전류 초과 시 전류
...
class MasterNode(Node):
    ...
    def control_loop(self):
        ...
        for i, dxl_id in enumerate(self.dxl_ids):
            ...
            # 슬레이브 전류 기반 피드백 (수정: 초과 시 master goal_pos를 slave_pos로 설정, goal_current=1193)
            if self.slave_currents is not None and self.slave_positions is not None:
                slave_curr = self.slave_currents[i]
                if slave_curr < min_curr or slave_curr > max_curr:
                    goal_pos = self.slave_positions[i]  # master goal_pos를 slave의 현재 pos로 설정
                    goal_current = STIFF_CURRENT  # 1193 적용
                    self.get_logger().warning(f"Slave motor ID {self.dxl_ids[i]} current exceeded limits: {slave_curr} (limits: {min_curr}~{max_curr}) - Setting master goal_pos to slave_pos {goal_pos} and feedback current {STIFF_CURRENT}")
