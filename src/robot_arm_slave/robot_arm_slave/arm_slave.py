...
class SlaveNode(Node):
    def __init__(self):
        ...
        for i, dxl_id in enumerate(self.dxl_ids):
            ...
            self.packetHandler.write1ByteTxRx(self.portHandler, dxl_id, 11, 5)  # 전류 기반 위치 제어 모드 설정
            self.packetHandler.write4ByteTxRx(self.portHandler, dxl_id, 112, PROFILE_VELOCITY)  # 프로파일 속도 설정
            self.packetHandler.write4ByteTxRx(self.portHandler, dxl_id, 108, PROFILE_ACCEL)     # 프로파일 가속도 설정
    ...
    def control_loop(self):                             # 주기적으로 호출되는 제어 루프 메서드
        ...
        for i, dxl_id in enumerate(self.dxl_ids):       # 각 모터에 대해 반복합니다.
            ...
            log_msg += f"ID{dxl_id} Pos: {pos} Present Current: {current}; "       # 로그에 모터 ID, 위치, 현재 전류
        msg = Int32MultiArray()                                 # 발행할 메시지 객체를 생성합니다.
        msg.data = positions + currents                         # 데이터를 연결합니다: 위치 배열 + 현재 전류 배열
